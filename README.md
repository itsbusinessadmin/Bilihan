...
- `page.js` — shared script for the privacy/terms/404 pages
- `consent.js` — cookie/storage consent banner
- `analytics.js` — consent-gated analytics loader (does nothing until configured)
- `styles.css` — UI
- `bilihan-logo.png` — Bilihan logo (also the fallback image for products)
...
## Launch checklist — what is already done

| # | Item | Status |
|---|------|--------|
| 1 | Privacy policy page | `privacy.html`, linked from the footer, the mobile menu, and the cookie banner |
| 2 | Terms & conditions page | `terms.html`, linked from the footer and the mobile menu |
| 3 | Secrets off the frontend | Only the Supabase anon key (public by design, guarded by RLS) and the public Apps Script endpoint ship to the browser; both are documented in `config.js` |
| 4 | Force HTTPS | `upgrade-insecure-requests` plus an `http:` → `https:` redirect on every page. **Also switch on GitHub Pages → Settings → Pages → Enforce HTTPS** |
| 5 | Cookie consent banner | `consent.js`; analytics stay off until the visitor accepts |
| 6 | Meta titles + descriptions | Unique title, description, and canonical URL on every page |
| 7 | Social preview image | `og-image.jpg` (1200×630) with Open Graph + Twitter card tags |
| 8 | Favicon | `favicon.ico`, PNG icons, Apple touch icon, and `site.webmanifest` |
| 9 | Sitemap + robots.txt | `sitemap.xml` and `robots.txt` (admin is disallowed) |
| 10 | Alt text on images | Every `<img>` has `alt`; decorative images use `alt=""` + `aria-hidden` |
| 11 | Compressed images | `bilihan-logo.png` 1.34 MB → 102 KB; header/footer use a 7 KB WebP mark |
| 12 | Page load speed | Preconnects to Supabase and the CDN, `loading="lazy"`, `fetchpriority` on the hero, explicit `width`/`height` to stop layout shift |
| 13 | Colour contrast | Every rendered text/background pair passes WCAG AA in both light and dark themes |
| 14 | Mobile friendly | Verified from 320 px to 1600 px |
| 15 | Custom 404 page | `404.html` |
| 16 | Broken links | All internal links and anchors verified |
| 17 | Form validation | Inline per-field errors with `aria-invalid`, plus server-side re-validation of stock and prices |
| 18 | Spam protection | Honeypot field, minimum form dwell time, and a 30-second order cooldown |
| 19 | Analytics | `analytics.js` — see "Turning on analytics" below |
| 20 | One clear call to action | "Shop Now" in the hero, "Start your order" in Contact, "Checkout" in the cart |
| — | No horizontal scroll | Verified at 320/360/390/414/560/768/820/1024/1280/1600 px with overflow clipping disabled |
| — | Mobile menu | Hamburger menu with Products, About, Contact, My Order, and the legal pages |
| — | Footer links | Shop, Legal, and Contact columns |
| — | Copyright year | Rendered in HTML and refreshed by JS |
...
| — | Success + error messages | Toasts on success, an alert banner plus per-field messages on failure |

## Before you go live

1. **Run the two new columns** in Supabase → SQL Editor if your database predates this change:
   ```sql
...
2. **Fill in Store Settings** in Admin: business name, contact number, contact email, Messenger,
   Instagram, and pickup location. Anything left at its seeded default is hidden from customers
...
5. **If you change domain**, update the canonical/Open Graph URLs in the three HTML pages,
   `robots.txt`, `sitemap.xml`, and `SITE_URL` in `config.js`.

## Turning on analytics

Analytics are off until you configure them, and are never loaded unless the visitor accepts the
cookie banner. Set `ANALYTICS` in `config.js` to one of:
...
customer name, phone number, address, or order contents ever reaches the analytics provider.

## Spam protection

The checkout form carries three client-side deterrents: a honeypot field no human can see, a
minimum dwell time before the form will submit, and a 30-second cooldown between orders from the
same browser. These stop casual bots but run in the browser, so a determined attacker can bypass
them. For a high-traffic launch, add a server-side control in front of `place_order` — Supabase
rate limiting, Cloudflare Turnstile, or an Edge Function that verifies a CAPTCHA token.
