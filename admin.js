...
function orders(m){
  const totals=A.data.orders.reduce((acc,o)=>{const st=o.payment_status||'Pending';acc.sell+=+o.total;if(st==='Paid')acc.paid+=+o.total;else acc.unpaid+=+o.total;(o.order_items||[]).forEach(i=>{const q=Number(i.qty||0),p=A.data.products.find(x=>x.id===i.product_id)||A.data.products.find(x=>String(x.name||'').toLowerCase()===String(i.product_name||'').toLowerCase());acc.original+=Number(i.original_price??p?.original_price??i.price??p?.price??0)*q;acc.interest+=Number(i.interest??p?.interest??0)*q});return acc},{sell:0,paid:0,unpaid:0,original:0,interest:0});
...
window.deleteOrder=async id=>{const o=A.data.orders.find(x=>x.id===id);if(!o)return alert('Order not found.');if(!confirm(`Delete Order #${o.order_code} permanently? This will also delete its Google Sheet row.`))return;const {error}=await db.from('orders').delete().eq('id',id);if(error)return alert(error.message);await deleteOrderFromGoogleSheet(o.order_code);await loadAll();renderShell()};async function deleteAllOrders(){if(!A.data.orders.length)return alert('There are no orders to delete.');if(!confirm(`Delete ALL ${A.data.orders.length} customer orders? This cannot be undone and will also delete their Google Sheet rows.`))return;if(prompt('Type DELETE ALL ORDERS to confirm:')!=='DELETE ALL ORDERS')return alert('Delete All cancelled.');const rows=[...A.data.orders];const {error}=await db.from('orders').delete().in('id',rows.map(o=>o.id));if(error)return alert(error.message);await Promise.allSettled(rows.filter(o=>o.order_code).map(o=>deleteOrderFromGoogleSheet(o.order_code)));A.orderFilter='all';await loadAll();renderShell();alert('All customer orders have been deleted.');}
function settings(m){
