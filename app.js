...
function openCheckout(){
  ...
  const fulfillmentField=fulfillmentValues.length===1?`<div class="field"><span>Receive order</span><div class="choice-value">${esc(fulfillmentValues[0])}</div><input type="hidden" name="fulfillment" value="${esc(fulfillmentValues[0])}"></div>`:`<label class="field">Receive order *<select name="fulfillment">${fulfillmentOptions}</select></label>`;
  ...
  const paymentField=paymentValues.length===1?`<div class="field"><span>Payment</span><div class="choice-value">${esc(paymentValues[0].label)}</div><input type="hidden" name="payment" value="${esc(paymentValues[0].value)}"></div>`:`<label class="field">Payment *<select name="payment">${paymentOptions}</select></label>`;
