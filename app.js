...
/* A product picture that shows its own loading state. Until the file arrives the
   tile shimmers; a product with no photo, or one whose photo fails, gets a quiet
   empty tile. Neither case falls back to the store logo any more: a logo in a
   product slot reads as a broken image rather than as "no photo yet". */
const BLANK_PIXEL='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
function productImageHtml(p,cls,extra=''){
  const url=String(p.image_url||'').trim();
  if(!url)return `<div class="${cls} is-empty" role="presentation"></div>`;
  return `<img class="${cls} is-loading" ${extra} src="${esc(url)}" alt="${esc(p.name)}">`;
}
/* Called after any markup containing product images is written. Images already in
   the browser cache are complete before this runs, so they are settled here rather
   than waiting for a load event that has already been and gone. */
function settleProductImages(root){
  (root||document).querySelectorAll('img.is-loading').forEach(img=>{
    /* The blank pixel a failure swaps in loads successfully a moment later, which
       would otherwise fire the load handler and mark a broken image as loaded. */
    let settled=false;
    const done=()=>{if(settled)return;settled=true;img.classList.remove('is-loading');img.classList.add('is-loaded')};
    const failed=()=>{if(settled)return;settled=true;img.classList.remove('is-loading');img.classList.add('is-empty');img.src=BLANK_PIXEL;img.alt=''};
    if(img.complete)return img.naturalWidth>0?done():failed();
